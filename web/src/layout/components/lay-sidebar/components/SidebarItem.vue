...
<template>
  <SidebarLinkItem v-if="hasOneShowingChild(item.children, item)
    ...
    " truncated class="!w-full !pl-4 !text-inherit">
  ...
  <el-sub-menu v-else ref="subMenu" teleported :index="resolvePath(item.path)" v-bind="expandCloseIcon">
    <template #title>
      ...
      <ReText v-if="layout === 'mix' && toRaw(item.meta.icon)
    ...
    offset: [0, -10],
    theme: tooltipEffect,
  }" class="!w-full !text-inherit" :class="{
    '!pl-4':
      layout !== 'horizontal'
      && isCollapse
      && !toRaw(item.meta.icon)
